...
Vector computeEulerAngles(Matrix& Q){
  ...
  // abc(0) = atan2(Q(1,2),Q(2,2));
  // double c2 = sqrt(Q(0,0)*Q(0,0) + Q(0,1)*Q(0,1));
  // abc(1) = atan2(-Q(0,2),c2);
  // double s1 = sin(abc(0)); double c1 = cos(abc(0));
  // abc(2) = atan2(s1*Q(2,0)-c1*Q(1,0), c1*Q(1,1)-s1*Q(2,1));
...
// dimensions.  
std::vector<Matrix> wigner_d(double theta, int L)
{
  int l, k, m;
