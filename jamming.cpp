...
int main(int argc,char** argv){
  ...
  Ylm = gsl_matrix_alloc(qThetas.size(), Ntot);
  Ylm_t = gsl_matrix_alloc(qThetas.size(), Ntot);
  Ylm_tt = gsl_matrix_alloc(qThetas.size(), Ntot);
  
  Ylm_p = gsl_matrix_alloc(qThetas.size(), Ntot);
  Ylm_pp = gsl_matrix_alloc(qThetas.size(), Ntot);
  Ylm_tp = gsl_matrix_alloc(qThetas.size(), Ntot);

  for (int qi=0; qi < qThetas.size(); qi++){
	...
	if (m<0){
	  gsl_matrix_set(Ylm,    qi, i, S2*plm[idx]*sin(absm*p)     );
	  gsl_matrix_set(Ylm_t,  qi, i, S2*(plm_1[idx])*sin(absm*p) );
	  gsl_matrix_set(Ylm_tt, qi, i, S2*(plm_2[idx])*sin(absm*p) );

	  gsl_matrix_set(Ylm_p,  qi, i, absm*S2*(plm[idx])*cos(absm*p) );
	  gsl_matrix_set(Ylm_pp, qi, i, -m*m*S2*(plm[idx])*sin(absm*p) );

	  gsl_matrix_set(Ylm_tp, qi, i, absm*S2*(plm_1[idx])*cos(absm*p));

	}
	if (m==0){
	  gsl_matrix_set(Ylm,    qi, i, plm[idx]   );
	  gsl_matrix_set(Ylm_t,  qi, i, plm_1[idx] );
	  gsl_matrix_set(Ylm_tt, qi, i, plm_2[idx] );	  
	}
	if (m>0){

	  gsl_matrix_set(Ylm,    qi, i, S2*plm[idx]*cos(m*p)     );
	  gsl_matrix_set(Ylm_t,  qi, i, S2*(plm_1[idx])*cos(m*p) );
	  gsl_matrix_set(Ylm_tt, qi, i, S2*(plm_2[idx])*cos(m*p) );

	  gsl_matrix_set(Ylm_p,  qi, i, -m*S2*(plm[idx])*sin(m*p)   );
	  gsl_matrix_set(Ylm_pp, qi, i, -m*m*S2*(plm[idx])*cos(m*p) );

          gsl_matrix_set(Ylm_tp, qi, i, -m*S2*(plm_1[idx])*sin(m*p) );
	}
	i ++;
      }
